...
export function getProductsDefult() {
  return async function (dispatch) {
    try {
      const productsDefault = await axios.get(
        process.env.REACT_APP_GET_ITEMS_DEFAULT
      );
      return dispatch({
...
export function getProductsBySearch(search) {
  return async function (dispatch) {
    try {
      const productsBySearch = await axios.get(
        `${process.env.REACT_APP_GET_ITEMS_BY_SEARCH}${search}`
      );
      return dispatch({
  ...
  return async function (dispatch) {
    try {
      const productDetails = await axios.get(
        `${process.env.REACT_APP_GET_PRODUCT_DETAILS}${id}`
      );
      return dispatch({
